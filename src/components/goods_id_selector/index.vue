        ...
        >
          ...
          <el-table-column label="全选" width="60">
            <template slot-scope="scope">
              <el-image style="width: 40px; height: 40px" :src="scope.row.picUrl" fit="cover">
  ...
  methods: {
    ...
    getGoodsList(page) {
      ...
      if(this.goodsTypeVal[0]) {
        formData.productGroup.categoryId = this.goodsTypeVal[0]
      }
      if(this.goodsTypeVal[1]) {
        formData.productGroup.subclassId = this.goodsTypeVal[1]
      }
      // formData.fromChannel = [1];  1-系统商品
    ...
    save() {
      this.$emit("submit", this.selectedGoodsID);
    }
  },
  filters: {
    priceFilter(val) {
...
<style>
@import url("../goods_selector/style/index.min.css");
</style>